            ...
            <div class="mx-auto max-w-2xl lg:max-w-5xl flex gap-6">
                <div class=" w-3 h-[full] shrink-0 overflow-y-hidden border-t-[6px] border-b-[6px] border-[#FFA132]"
                    ...
                    <template x-for="(row, index) in rows" class="flex w-full bg-yellow-500 text-white"
                        ...
                        <div class="flex">
                            <template x-for="(c, cindex) in cols" :key="cindex">
                                <div :class="visibility?.[index]?.[cindex] == 1 ? 'bg-orange' : 'bg-transparent'"
                ...
                <div class="max-w-2xl">
                    <h1 class=" font-bold text-zinc-800 sm:text-6xl dark:text-zinc-100">
                        <span class="inline-block tracking-tighter pr-1 text-white ">{!! markdown(config('site.home.headline')) !!}</span>
                    </h1>
                    ...
                    <div class="hidden mt-6 flex gap-6"><a class="group -m-1 p-1" aria-label="Follow on X"
                            target="_blank" href="{{ config('site.social-links.twitter') }}">
